...
public class AuthRepository {
        ...
        firebaseAuth.signInWithCredential(credential)
                .addOnCompleteListener(task -> {
                    if (task.isSuccessful()) {
                        FirebaseUser firebaseUser = firebaseAuth.getCurrentUser();
                        if (firebaseUser != null) {
                            // JANGAN langsung onSuccess. Cek dulu ke database.
    ...
    public void sendOtp(String phoneNumber, Activity activity,OtpCallback otpCallback) {
        ...
        PhoneAuthOptions options = PhoneAuthOptions.newBuilder(firebaseAuth)
                .setPhoneNumber(formattedPhone)       // Nomor HP (+62xxx)
                .setTimeout(60L, TimeUnit.SECONDS) // Timeout
                .setActivity(activity)             // Activity
                .setCallbacks(callbacks)           // Callback di atas
                .build();
    ...
    // Fungsi tambahan untuk Cek Database
        ...
        firestore.collection("users").document(uid)
                ...
                .addOnSuccessListener(documentSnapshot -> {
                    if (documentSnapshot.exists()) {
                        // KONDISI 1: User Lama (Sudah punya data)
                        // Langsung kabari ViewModel sukses
                        callback.onSuccess(firebaseUser);
                    } else {
                        // KONDISI 2: User Baru (Belum punya data di Firestore)
                        // Kita harus simpan data Google mereka ke Firestore dulu
...
//    }
    ...
    public void getUserData(String uid, UserDataCallback callback) {
        firestore.collection("users").document(uid)
                ...
                .addOnSuccessListener(documentSnapshot -> {
                    if (documentSnapshot.exists()) {
                        // Ubah dokumen JSON dari Firestore menjadi objek User Java
